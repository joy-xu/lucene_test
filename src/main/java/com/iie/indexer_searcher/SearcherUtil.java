...
public class SearcherUtil {
	...
	public IndexSearcher getSearcher(Directory directory) throws Exception {
		/*if (reader == null) {
			reader = IndexReader.open(directory);
		} else {
			IndexReader tr = IndexReader.openIfChanged(reader, true);
	...
	public void searchPage(String query,int pageIndex,int pageSize) throws Exception {
		try {//全部读到内存中处理
			Directory dir = FileIndexUtil.getDirectory();
			IndexSearcher searcher = getSearcher(dir);
			QueryParser parser = new QueryParser(Version.LUCENE_35,"content",new StandardAnalyzer(Version.LUCENE_35));
			Query q = parser.parse(query);
			TopDocs tds = searcher.search(q, 500);//全部读到内存中处理
	...
	public void searchPageByAfter(String query,int pageIndex,int pageSize) throws Exception {
		try {
			Directory dir = FileIndexUtil.getDirectory();
			IndexSearcher searcher = getSearcher(dir);
			QueryParser parser = new QueryParser(Version.LUCENE_35,"content",new StandardAnalyzer(Version.LUCENE_35));
